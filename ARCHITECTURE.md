...
│   ├── draw_objects.cpp    # ESP 绘制入口、骨骼缓存发布
│   ├── draw_menu.cpp       # ImGui 菜单 UI
│   ├── auto_aim.cpp        # 自瞄控制器
│   ├── game_frame_reader.cpp # 游戏帧快照读取（VP / DeltaTime / GFrameCounter）
...
│   ├── draw_objects.h      # ESP 绘制接口、骨骼快照接口
│   ├── draw_menu.h         # 菜单绘制接口
│   ├── game_frame_reader.h # 游戏帧读取接口
...
ReadThread
    ...
    │
    └── 发布到共享状态:
          ├── ClassifiedActorsSnapshot (11 类 Actor 的不可变快照)
          └── GameFrameData (由 game_frame_reader.cpp 在渲染侧读取)
```

关键数据结构：
- `CachedActor` — 每个 Actor 的缓存（地址、骨骼映射、分类信息）
- `ClassifiedActorsSnapshot` — 读取线程整帧构建并一次性原子发布的只读分类快照
- `BoneScreenData` — 屏幕空间骨骼坐标（含骨骼缓存指针，供 auto-aim 实时刷新）
- `GameFrameData` — 单帧游戏快照

### 渲染管线

**文件**：`VulkanApp.cpp`、`ImGuiLayer.cpp`、`main.cpp`、`game_frame_reader.cpp`
...
- **骨骼快照发布**：主线程每帧构建 `BoneScreenCacheSnapshot`，完成绘制后一次性原子发布给自瞄线程
...
AutoAimController::UpdateThreadFunc()
    ...
    │
    ├── 2. 选择目标 (SelectTarget)
    │     ├── 获取骨骼屏幕快照 (GetBoneScreenCacheSnapshot)
...
```
    ...
    │              共享数据                          │
    │                                               │
    │  ClassifiedActorsSnapshot (atomic shared_ptr) │
    │  BoneScreenCacheSnapshot (atomic shared_ptr)  │
...
读取线程整轮扫描后构建新的 `ClassifiedActors`，再通过 `atomic shared_ptr` 一次性发布；渲染线程和自瞄线程始终只读快照，不参与合并和拷贝。骨骼屏幕坐标同样采用整帧构建、整帧发布，避免热路径上的 `mutex + unordered_map` 整表复制。
...
```
BoneScreenCacheSnapshot (渲染线程填充并整帧发布)
    │
